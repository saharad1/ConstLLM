"""
Script for evaluating trained SFT models by computing attributions on test datasets.
This script loads a trained model and runs the same evaluation process as in data collection.
"""

# Workaround for DGX nodes with broken GPUs: the broken GPUs corrupt the CUDA
# runtime state so that cudaGetDeviceCount() always fails, and triton (imported
# by bitsandbytes) crashes because it finds 0 active drivers.
# Fix: (1) mock triton so bitsandbytes can import without crashing, and
#      (2) pre-init CUDA via ctypes to establish a context on the working GPU.
import ctypes
...
def _mock_triton():
    """Pre-populate sys.modules with mock triton modules to prevent bitsandbytes/triton crash."""
    if "triton" in sys.modules:
        return
    submodules = [
        "triton", "triton.language", "triton.runtime",
        "triton.runtime.autotuner", "triton.runtime.driver",
    ]
    ...
    print("[GPU fix] Mocked triton module (not needed without quantization)")

def _fix_cuda_init():
    """Force CUDA initialization via ctypes, bypassing PyTorch's problematic lazy init."""
    try:
        ...
        cudart.cudaGetLastError()
        print("[GPU fix] CUDA context initialized successfully via ctypes")
    except Exception as e:
        print(f"[GPU fix] Could not pre-initialize CUDA via ctypes: {e}")
...
def eval_trained_sft(
    ...
    """
    Evaluate a trained SFT model on a test dataset by computing attributions.
    ...
    llm_analyzer = LLMAnalyzer(model_id=model_path_str, device_map=device_map, temperature=temperature, use_quantization=use_quantization)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no_quantization",
        action="store_true",
        help="Disable 4-bit quantization when loading the model (useful when GPU driver has issues with bitsandbytes)",
    )
