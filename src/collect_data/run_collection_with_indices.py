...
def setup_collection_with_indices_run_environment(dataset_name, model_id, attribution_method_name, resume_run=None):
    ...
    output_dir = Path(f"data/collection_with_indices/{dataset_name}/{safe_model_id}")
...
def run_collection_with_indices(
    model_id,
    dataset_name,
    indices_file,
    ...
    if use_wandb:
        wandb.init(
            project="constllm_collection_with_indices",
            name=run_name,
            config={
                "model_id": model_id,
    ...
    # Final summary
    print(f"\nData collection with indices completed!")
