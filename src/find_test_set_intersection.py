...
def create_consistent_test_set(test_files: List[Path], output_dir: Path, dataset_name: str = "unknown") -> None:
    """
    ...
    # Save intersection info with scenario IDs
    intersection_info = {
    ...
    intersection_file = output_dir / f"{dataset_name}_intersection_testset_{len(intersection_scenario_ids)}.json"
    with open(intersection_file, "w") as f:
        json.dump(intersection_info, f, indent=2)

    print(f"Saved intersection test set to: {intersection_file}")
    print(f"Intersection contains {len(intersection_scenario_ids)} scenario IDs")

    print(f"\nIntersection test set created successfully!")
    print(f"Use the scenario IDs in this file for consistent evaluations.")
...
def main():
    ...
    create_consistent_test_set(test_files=test_files, output_dir=output_dir, dataset_name=dataset_name)
